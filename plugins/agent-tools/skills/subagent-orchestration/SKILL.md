...
- 일반 작업: 사전 검증 2개, 사후 검증은 `architect`와 `reviewer` 2개를 병렬로 사용한다.
- 고위험 작업: 사전 검증은 최대 3개, 사후 검증은 기본 2개에 전문 검증 1개를 추가할 수 있다.
- 고위험이면서 전문 영역 리스크가 큰 작업도 사후 검증은 최대 3개까지만 사용한다.
...
4. 실행이 가능하고 병렬 이득이 있으면 아래 순서로 진행한다.
   ...
   - 일반 작업은 사후 검증으로 `architect`와 `reviewer`를 동시에 실행해 최종 확인한다.
   - 고위험 작업은 전문 영역 리스크가 크면 사후 검증을 최대 3개까지 늘릴 수 있다.
   - 사후 검증 의견을 반영해 코드가 바뀌면 메인 에이전트가 실행 검증을 다시 수행한다.
5. 각 서브에이전트는 역할이 겹치지 않게 분리하고, 같은 파일을 중복 수정하지 않도록 책임 범위를 먼저 적는다.
6. 사전 검증 결과는 메인 구현 전에 반드시 회수하고, 사후 검증 결과는 최종 응답 전에 반드시 회수한다.
...
9. 전역 12개 전문 롤은 후보군이며, 매 작업마다 모두 사용하지 않는다. 기본은 사전 검증 2개와 사후 검증 `architect` + `reviewer` 2개이며, 고위험 작업만 전문 사후 검증을 1개 추가한다.
...
- 사후 검증 A (`architect`): 구조, 책임 경계, 의존성, 불필요한 계층과 구조 단순화 가능성 확인
- 사후 검증 B (`reviewer`): 중복, dead code, 기존 구현 재사용, 불필요한 코드 추상화와 최종 diff 확인
- 추가 사후 검증: 고위험 작업에서만 전문 영역 회귀를 별도 역할로 확인
