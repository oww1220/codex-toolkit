...
def build_description(cfg):
    if cfg.get("description"):
        return cfg["description"].strip()
    purpose = (cfg.get("purpose") or "").strip()
    if not purpose:
        die("config needs `description` or `purpose`")
    parts = [purpose]
    if cfg.get("triggers"):
        parts.append("트리거 — " + ", ".join(f'"{t}"' for t in cfg["triggers"]) + ".")
    if cfg.get("anti_triggers"):
        parts.append("비-트리거: " + "; ".join(cfg["anti_triggers"]) + ".")
    return " ".join(parts)


def render_thin(cfg, name):
...
def render_body(cfg, name):
    title = name.replace("-", " ").title()
    S = [f"# {title}", ""]

    S += ["## 무엇을 / 언제 (한 줄)", cfg.get("one_liner") or "[이 스킬이 하는 일 한 문장 — description과 일관되게.]", ""]
    ...
    if len(modes) > 1:
        ...
        for m in modes:
            S.append(f"| **{m.get('label', '[모드]')}** | {m.get('start', '-')} | {m.get('when', '[조건]')} |")
        S += ["판단: [한 줄 휴리스틱]. **애매하면 `request_user_input`으로 묻는다 — 임의로 고르지 말 것.**", ""]

    if cfg.get("orchestrator") or cfg.get("routed_by") == "orchestrator":
        pc = cfg.get("pipeline") or {}
        S += ["## Pipeline Context", "| 항목 | 내용 |", "|------|------|",
    ...
    if gate.get("type") == "script":
        ...
        S.append(f"{i}. 검증(게이트): `{cmd}` 실행 — exit 0이어야 다음 단계. "
    ...
    if refs:
        ...
        for r in refs:
            S.append(f"| {r.get('summary', '[주제]')} | references/{r['file']} | {r.get('when', '[조건]')} |")
        S += ["> 각 reference 파일은 첫 줄을 `> **로드 시점**: <같은 조건>`으로 시작해 트리거를 메아리친다.", ""]

    S += ["## 출력 위치 (필수)"]
    for o in (cfg.get("output_required") or ["`output/{slug}/` 등 cwd 상대 + `mkdir -p` 가드"]):
        S.append(f"- ✅ {o}")
    S += ["- ⛔ 채팅 코드 덤프 / `/Users/idong-geol/.codex/skills/`(설치 디렉터리) 안 임시 산출물 / 홈 절대경로"]
    return "\n".join(S).rstrip() + "\n"


def _gate_stub(cmd):
    m = re.search(r"scripts/([A-Za-z0-9_./\-]+)", cmd or "")  # '/' so subdir paths (scripts/sub/x.sh) survive
    if not m:
        return None, None
    fname = m.group(1)
...
def main():
    ...
    if basename != name:
        print(f"scaffold: WARN: dirname '{basename}' != name '{name}' (audit will flag)", file=sys.stderr)
    ...
    if thin and cfg.get("thin_steps"):
    ...
    deferred = [sub for sub, flag in (("scripts", "has_scripts"), ("templates", "has_templates"))
                if cfg.get(flag) and not os.path.isdir(os.path.join(skill_dir, sub))]
    ...
    print("\n출하 게이트: python3 /Users/idong-geol/.codex/skills/skill-enhancer-codex/scripts/audit.py", skill_dir)


def _audit_gaps(cfg, thin):
