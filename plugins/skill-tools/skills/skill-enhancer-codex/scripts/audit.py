...
import refs_check  # noqa: E402

KEBAB_RE = re.compile(r"^[a-z0-9]+(-[a-z0-9]+)*$")
GATE_TOKENS = ["통과 조건", "Phase 전환 조건", "✅ 통과", "GATE", "pass_threshold",
...
def parse_frontmatter(text):
    """Light YAML-ish parser: returns (fields:dict, body:str, fm_present:bool).
    Captures top-level scalar/block/list values well enough for linting."""
...
def audit(skill_dir):
    ...
    if not os.path.isfile(skill_md):
        ...
        return {"skill": dirname, "score": 0, "results": R, "passed": passed}

    text = open(skill_md, encoding="utf-8", errors="replace").read()
    fields, body, fm_ok = parse_frontmatter(text)
    line_count = text.count("\n") + 1
    name = unquote_scalar(fields.get("name", ""))
    desc = unquote_scalar(fields.get("description", ""))

    if fm_ok:
        passed.append("frontmatter block parses")
    elif text.lstrip().startswith("---"):
        add(R, "HARD", "frontmatter", "frontmatter opens with `---` but has no closing `---` (malformed YAML)")
    else:
    ...
    if desc:
        ...
        if combined > 1536:
            add(R, "WARN", "desc-len", f"description(+when_to_use) {combined} chars > 1536 — truncated in the skill listing")
        else:
    ...
    if not has_trigger_field and quoted == 0:
        add(R, "WARN", "discovery", "auto-invocable skill has no quoted triggers in description — weak self-activation")
    else:
    ...
    if re.search(r"Skip when|비-트리거|don't use when|사용 금지|안 함", text, re.I):
        ...
        add(R, "INFO", "anti-trigger", "no 'Skip when / 비-트리거' anti-trigger clause (recommended)")

    # 9 WARN line count
    if line_count > 500:
        add(R, "WARN", "length", f"SKILL.md is {line_count} lines (>500) — move detail to references/")
    else:
    ...
    if rc["untriggered_files"]:
        add(R, "WARN", "refs-untriggered", "references with no load trigger (dead weight, or output-template/shared): " + ", ".join(rc["untriggered_files"]))
    if rc.get("external_or_optional"):
        add(R, "WARN", "refs-external", " refs absent but cross-skill or intentionally-optional (verify): " + ", ".join(rc["external_or_optional"]))
    if rc["dangling_pointers"]:
        add(R, "HARD", "refs-dangling", "read pointers to missing files (no fallback, local): " + ", ".join(rc["dangling_pointers"]))
    if not rc["untriggered_files"] and not rc["dangling_pointers"] and not rc.get("external_or_optional"):
    ...
    for raw in set(re.findall(r"scripts/[A-Za-z0-9_./\-]+", body)):
        ...
        if "." in os.path.basename(tok) and not os.path.exists(os.path.join(skill_dir, tok)):
            add(R, "WARN", "script-missing", f"body references {tok} but no such file in this skill (shared/sibling?)")
    ...
    if len(phase_heads) >= 2:
        for idx in phase_heads:
            ...
            if not any(tok in window for tok in GATE_TOKENS):
                add(R, "WARN", "gate", f"phase heading near line {idx+1} has no gate token within 25 lines")
    ...
    for i, ln in enumerate(body_lines):
        for m in _home.finditer(ln):
            if _wkw.search(ln[max(0, m.start() - 40):m.end() + 40]):
                add(R, "WARN", "output-path", f"line {i+1}: writes into install/home dir — use cwd-relative paths")
                break
            ...
            for fn in files:
                if fn.lower().endswith((".html", ".htm")):
                    ...
                    for term in BANNED_OUTPUT_TERMS:
                        if term in content:
                            add(R, "WARN", "banned-term", f"templates/{fn}: leaks process term '{term}' into output")
...
def main():
    ...
    if "--json" in sys.argv:
        ...
        for k, label in (("hard_fails", "HARD"), ("warnings", "WARN"), ("infos", "INFO")):
            for r in rep.get(k, []):
