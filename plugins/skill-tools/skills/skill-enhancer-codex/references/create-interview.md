...
2. `python3 /Users/idong-geol/.codex/skills/skill-enhancer-codex/scripts/scaffold.py <config.json> <skill_dir>` 실행.
3. 출력의 **"남은 인터뷰 항목"** 이 있으면 그 항목만 다시 묻고 config에 추가 → 재실행.
4. **`python3 /Users/idong-geol/.codex/skills/skill-enhancer-codex/scripts/audit.py <skill_dir>`** → HARD 0이면 완료. WARN이 남으면 해당 필드를 더 받아 채운다.
