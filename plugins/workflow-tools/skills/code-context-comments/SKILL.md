...
사용자가 범위를 명시하지 않았다면 작업 전에 다음 중 하나를 선택하게 한다.

1. **현재 변경분만**: Git diff의 추가·수정 구문과 이를 이해하는 데 필요한 인접 코드
2. **전체 대상**: 사용자가 지정한 파일·디렉터리·모듈의 전체 코드

사용자가 이미 범위를 명시했다면 다시 묻지 않고 그 범위를 따른다. 선택한 범위는 조사·작성·사후 검증에 동일하게 적용한다.
