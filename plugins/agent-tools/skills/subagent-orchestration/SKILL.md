...
현재 세션의 도구 목록에서 `spawn_agent`와 결과 회수에 필요한 협업 도구의 노출 여부를 먼저 확인한다.
...
시작 전에 반드시 현재 세션에서 서브에이전트 실행 도구를 먼저 확인하고, 실제 `spawn_agent` 입력 스키마를 따른다.

현재 세션에서 확인해야 할 기능은 다음과 같다. 도구명은 세션마다 다를 수 있다.

- 에이전트 생성
- 완료 대기와 결과 회수
- 실행 중 메시지 또는 후속 작업 전달
- 중단이나 정리가 필요한 경우의 수명주기 제어

## Agent Definition Source

- 전문 롤을 구성하기 전에는 반드시 이 `SKILL.md`와 같은 디렉터리의 `agents/*.toml` 정의를 먼저 확인한다.
- `spawn_agent` 도구 설명은 현재 세션의 실행 가능한 입력 스키마를 확인하는 용도로 쓰고, 전문 페르소나의 실제 책임과 경계는 스킬에 번들된 `agents/*.toml`을 기준으로 맞춘다.
- 도구가 제공하는 기본 롤은 보조 역할로만 쓰고, 전문 롤 12개와 혼동하지 않는다. TOML 전문 롤은 `Agent label`과 메시지의 책임 범위에 매핑한다.

## Global Role Roster

TOML 기준 전문 책임 롤은 다음 12개다. 실제 생성 인자는 현재 세션의 도구 설명을 따른다.

- `architect`: 구조, 책임 경계, 의존성, side effect 분석
...
- `reviewer`: 최종 diff와 코드 단위 리팩토링 리뷰

## Risk-Based Agent Count

검증 서브에이전트 수는 작업 위험도에 따라 조정한다. 4개 이상 필요해 보이면 검증 수를 늘리기보다 작업을 더 작은 단위로 분할한다.

- 일반 작업: 가장 관련 있는 사전 검증 1개를 사용하고, 독립 검증이 필요할 때만 사후 검증 1개를 추가한다.
- 고위험 작업: 사전 검증은 최대 3개, 사후 검증은 독립 전문 검증이 필요할 때만 최대 2개를 사용한다.
...
실제로 역할을 띄울 때는 아래처럼 `spawn_agent`를 명시하고, 전문 역할명은 `Agent label`과 메시지 책임 범위에 적는다. 아래 예시는 `fork_turns`를 지원하는 현재 협업 도구 기준이며, 실제 스키마가 다르면 그 규약을 따른다.

```text
spawn_agent({ task_name: "architect", fork_turns: "all", message: "Agent label: [architect]\n구조, 책임 경계, 의존성, side effect와 단순화 가능성만 검토해줘. 구조 영향이 없으면 없다고 보고해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "planner", fork_turns: "all", message: "Agent label: [planner]\n요구사항, 범위, 우선순위 정리 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "frontend", fork_turns: "all", message: "Agent label: [frontend]\nNext.js, React Native, Vue, Storybook, component architecture 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "backend", fork_turns: "all", message: "Agent label: [backend]\nNestJS API, module, service, dto 설계 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "db_designer", fork_turns: "all", message: "Agent label: [db_designer]\nSupabase, Prisma relation 설계 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "publishing", fork_turns: "all", message: "Agent label: [publishing]\nWebpack, EJS, SCSS, Storybook publishing specialist 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "accessibility", fork_turns: "all", message: "Agent label: [accessibility]\n웹/앱 접근성, 키보드 조작, ARIA, focus, semantic markup 검토 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "security", fork_turns: "all", message: "Agent label: [security]\n인증/권한, secret, 입력 검증, 개인정보/결제 보안 리스크 검토 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "devops_ci", fork_turns: "all", message: "Agent label: [devops_ci]\nGitHub Actions, 배포, Docker, env, 패키지 매니저, CI 실패 분석 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "designer", fork_turns: "all", message: "Agent label: [designer]\n사용자 흐름, 정보 구조, 화면 위계, UX writing, 제품 디자인 판단 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "tester", fork_turns: "all", message: "Agent label: [tester]\nlint, build, browser smoke, regression 확인 관점만 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
spawn_agent({ task_name: "reviewer", fork_turns: "all", message: "Agent label: [reviewer]\n최종 diff의 중복, dead code, 기존 구현 재사용, 불필요한 추상화와 코드 품질을 검토해줘. 직접 파일을 수정하지 말고, 결론 3줄과 리스크 1개로 줘." })
```

## Execution Rule

1. 먼저 현재 세션에서 서브에이전트 실행 도구를 찾고, 실제로 spawn 가능한지 확인한다.
2. 생성, 대기, 메시지 전달, 필요시 중단에 해당하는 실제 도구와 입력 스키마를 확인한다.
3. 실행이 불가능하면 이 스킬은 분기 판단까지만 하고 단일 에이전트로 처리한다.
4. 실행이 가능하고 병렬 이득이 있으면 아래 순서로 진행한다.
   - 일반 작업은 가장 관련 있는 사전 검증 서브에이전트 1개를 실행한다.
   - 고위험 작업은 사전 검증을 최대 3개까지 늘릴 수 있다.
   - 사전 검증 결과를 반드시 회수한다.
   - 회수한 검증 결과를 합쳐 현재 메인 에이전트가 실제 구현을 수행한다.
   - 메인 에이전트가 구현 완료 후 기본 테스트, lint, build 등 저장소에 맞는 실행 검증을 수행한다.
   - 구조 또는 회귀 위험이 남을 때만 가장 관련 있는 사후 검증 1개를 실행한다.
   - 고위험 작업은 독립 전문 검증이 필요하면 사후 검증을 최대 2개까지 늘릴 수 있다.
   - 사후 검증 의견을 반영해 코드가 바뀌면 메인 에이전트가 실행 검증을 다시 수행한다.
5. 각 서브에이전트는 역할이 겹치지 않게 분리하고, 같은 파일을 중복 수정하지 않도록 책임 범위를 먼저 적는다.
6. 사전 검증 결과는 메인 구현 전에 반드시 회수하고, 사후 검증을 실행했다면 최종 응답 전에 반드시 회수한다.
7. 실행 중 추가 지시가 필요하면 현재 세션의 메시지 또는 후속 작업 전달 도구를 쓴다. 명시적인 종료 도구가 있을 때만 별도 정리한다.
8. 전역 12개 전문 롤이 실제로 맞는 경우에는, 기본 4단계 흐름 안에서 해당 롤을 `Agent label`과 메시지 책임 범위로 우선 배치한다.
   - 구조/경계 불명확 또는 구현 후 구조 리팩토링 검토: `architect`
   - 요구사항/범위 불명확: `planner`
...
9. 전역 12개 전문 롤은 후보군이며, 매 작업마다 모두 사용하지 않는다. 기본은 가장 관련 있는 사전 검증 1개이며, 사후 검증은 독립 검증이 필요할 때만 추가한다.
...
- 사후 검증: 구조 또는 회귀 위험에 맞는 역할 1개를 선택하고, 고위험 작업에서만 최대 2개로 늘린다.

## Spawn Guidance

- 도구 탐색이 끝나기 전에는 spawn을 시작하지 않는다.
- 현재 도구가 지원하면 전체 대화 맥락을 넘기는 옵션을 기본으로 사용한다.
...
- 도구 스키마와 무관하게 메시지 첫 줄의 `Agent label: [role]`은 유지한다.
- 모든 검증, 리뷰 계열에는 "직접 파일을 수정하지 말고, 분석 결과와 권장 수정 방향만 반환한다"를 함께 전달한다.
...
- 스킬에 번들된 `agents/*.toml`을 전문 롤의 기준으로 사용한다.
- 새 롤이 필요하면 번들 내부 `agents/*.toml`과 `SKILL.md`를 함께 갱신한다.
- repo 전용 경로, 검증 명령, 문서 규칙은 특정 저장소의 `AGENTS.md`와 `docs/`를 따른다.
- 서브에이전트는 분석, 검증, 리뷰만 수행하며 직접 파일을 수정하지 않는다.
- 사전 검증 결과 없이 메인 구현을 시작하지 않는다.
- 사후 검증을 실행했다면 결과 없이 최종 완료로 판단하지 않는다.
